...
function get_attribute_type($type=''){
    ...
    static $_type = array(
		...
		'functions' =>  array('函数类型','varchar(50) NOT NULL'),
    );
...
function parse_field_functions($string) {
...
function get_user(){
  $pre = C("DB_PREFIX");
  $author_arr = M("document_author as da")->join($pre."document as d on da.id = d.id")->field("da.id,d.title")->select();
  if($author_arr){
...
function get_customer(){
  $pre = C("DB_PREFIX");
	$customer_arr = M("document_customer as dc")->join($pre."document as d on dc.id = d.id")->field("dc.id,d.title")->select();
...
function get_categorys(){
  $category = get_category(intval($_REQUEST["cate_id"]));

  $pid = isset($category["pid"])?$category["pid"]:1;
  $category_arr = M("category")->where("pid=".$pid)->field("id,title,pid")->select();
  if($category_arr){
