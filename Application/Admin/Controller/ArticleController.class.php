...
class ArticleController extends AdminController {
 ...
 /**
    ...
    public function edit(){
        ...
        $data["categorys_id"] = $data["category_id"];//默认给当前分类id给   
        if(!$data){
       ...
       if($data['model_id'] == C("RUN_MODELID")){
          $tag_arr = M("keywords")->select();
    ...
    public function update(){
        ...
        $document   =   D('Document');
        $_POST["category_id"] = isset($_POST["categorys_id"])?$_POST["categorys_id"]:$_POST["category_id"];//将新增的分类字段的值赋给category_id
        ...
        if(!$res){
            ...
            if($_POST["model_id"] == C("RUN_MODELID")){//关键词操作
              
              //$keywords_arr = explode(",",$_POST["keywords"]);
              //$keywords_arr = preg_split('/[\s,]+/', $_POST["keywords"])    
             // print_r($keywords_arr) ;
              $str = $_POST["keywords"];
              $keywords_arr = preg_split("/[\s,，]+/", $str);
     

              if(count($keywords_arr) == 1){
                $map["tag_name"] = $_POST["keywords"];
                $result = M("keywords")->where($map)->find();
                if(empty($result)){
                  $keywords_id = M("keywords")->add(array("tag_name"=>$_POST["keywords"],"article_id"=>$res['id'],"create_date"=>time()));
                }else{
                  $keywords_id = $result["id"];
                }
                $tags = M("tags")->where(array("keywords_id"=>$keywords_id,"article_id"=>$res['id']))->find();
                if(empty($tags)){
                   M("tags")->add(array("keywords_id"=>$keywords_id,"article_id"=>$res['id'],"create_at"=>date("Y-m-d H:i:s")));
                }
              }else{
                foreach($keywords_arr as $keyword){
                  $map["tag_name"] = $keyword;
                  $result = M("keywords")->where($map)->find();//如果找到关键词
                  if(empty($result)){
                    $keywords_id = M("keywords")->add(array("tag_name"=>$keyword,"article_id"=>$res['id'],"create_date"=>time()));
                  }else{
                    $keywords_id = $result["id"];
                  }
                  $tags = M("tags")->where(array("keywords_id"=>$keywords_id,"article_id"=>$res['id']))->find();
