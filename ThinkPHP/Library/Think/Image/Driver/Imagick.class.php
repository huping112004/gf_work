...
class Imagick{
     ...
     * @param  string  $font   字体路径
     * @param  integer $size   字号
     * @param  integer $width  文字框宽度
     * @param  integer $height 文字写高度
     ...
     * @param  integer $angle  文字倾斜角度
     */
    public function box_text($text, $font, $size, $width, $height, $align, $color = '#00000000', 
        $locate = Image::IMAGE_WATER_SOUTHEAST, $offset = 0, $angle = 0){
        //资源检测
        if(empty($this->img)) E('没有可以被写入文字的图像资源');
        if(!is_file($font)) E("不存在的字体文件：{$font}");
        ...
        $wrap_contents = $this->wrapContent($text, $font, $size, $width, $angle);
        $line_x = $x;
        $line_y = $y;
        foreach($wrap_contents as $key => $line){
            switch($align){
            case "center":
              $line_x = ($width - $line["width"])/2 + $x;
              break;
            case "right":
              $line_x = $width + $x - $line["width"];
              break;
            default:
              $line_x = $x;
              break;
            ...
            $this->text($line["content"], $font, $size, $color, $line_locate, $offset, $angle);
   ...
   */ 
   private function wrapContent($text, $font, $size, $width, $angle){
        ...
        foreach ($letter as $l) {
          ...
          $metrics = $this->img->queryFontMetrics($draw, $teststr);
          
          // 判断拼接后的字符串是否超过预设的宽度
          $line_width = $metrics['textWidth'];
          $line_height = $metrics['textHeight'];

          if ($line_width >= $width){
            $r_idx++;
            $line_content = $l;
            $metrics = $this->img->queryFontMetrics($draw, $line_content);
            // 判断拼接后的字符串是否超过预设的宽度
            $line_width = $metrics['textWidth'];
            $line_height = $metrics['textHeight'];
          }else{
          ...
          $lines[$r_idx] = array("content" => $line_content, "width" => $line_width, "height" => $line_height);
        }
        return $lines;
    }
